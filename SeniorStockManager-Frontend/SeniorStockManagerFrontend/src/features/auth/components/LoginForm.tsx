...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (status === 'ok' && identity) {
      login(identity);
      // §7.4 — prioridade: destino interno preservado por RequireAuth (mais
      // específico, nunca alcançável por URL) > returnTo validado da query
      // string (entrada cruzada — portal ou link direto, §4.5/§7.4) >
      // destino padrão do app.
      //
      // from usa navigate() (SPA, rápido) porque é sempre uma rota deste
      // próprio roteador. crossAppReturnTo usa window.location.assign()
      // (navegação de página inteira) porque portal/care/stock são três
    ...
    // §7.4 — o returnTo pendente precisa sobreviver à etapa de MFA.
