...
// serviceUtils.test.ts, capturando o callback que registerUnauthorizedHandler
// recebe pra exercitar a lógica de 401 sem precisar testar o interceptor do axios
// em si (baixo risco, só repassa a chamada). Mesmo teste da §10 já aplicado no
...
function Consumer() {
  ...
  return (
    <div>
      ...
      <span data-testid='has-religion-read'>
        {hasPermission('Religion', 'read') ? 'yes' : 'no'}
      </span>
      <span data-testid='has-religion-write'>
        {hasPermission('Religion', 'write') ? 'yes' : 'no'}
      </span>
    </div>
  );
}
...
const identity = {
  ...
  effectivePermissions: [{ resource: 'Religion', action: 'read' }],
};
...
describe('AuthProvider', () => {
  ...
  it('restores the session from GET /auth/me and exposes hasPermission against EffectivePermissions', async () => {
    ...
    expect(screen.getByTestId('has-religion-read').textContent).toBe('yes');
    expect(screen.getByTestId('has-religion-write').textContent).toBe('no');
  });
